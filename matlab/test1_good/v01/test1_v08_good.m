...
po1=get(h1,'position');
plot(t_markov, reward_markov, 'r-o', t_greedy, reward_greedy, 'b-*', 'MarkerSize', 5);
xlabel('Time (s)');
ylabel('Reward ($)');
legend('Markov', 'Greedy');
...
po2=get(h1,'position');
% subplot(3, 1, 2);
plot(t_markov, price_markov, 'r-o', t_greedy, price_greedy, 'b-*', 'MarkerSize', 5);
xlabel('Time (s)');
...
po3=get(h1,'position');
% subplot(3, 1, 3);
plot(t_markov, time_markov, 'r-o', t_greedy, time_greedy, 'b-*', 'MarkerSize', 5);
xlabel('Time (s)');
ylabel('Time Delay (s)');
legend('Markov', 'Greedy');
% x_size = (max(max(t1), max(t2)))+2;
